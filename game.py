        ...
        for dx, dy in directions:
            ...
            if (0 <= ax1 < GRID_SIZE and 0 <= ay1 < GRID_SIZE and
                ...
                if (self.board.get_piece((ax1, ay1)) == opponent and
                    ...
                    result.append(((ax1, ay1), f"规则a: 方向({dx},{dy}) 己方-敌方-己方"))

            # 规则b: 敌方-己方-敌方 (新位置是中间的己方)
            # 检查: 反方向1格是敌方，正方向1格是敌方
            bx1, by1 = nx - dx, ny - dy
            bx2, by2 = nx + dx, ny + dy
            if (0 <= bx1 < GRID_SIZE and 0 <= by1 < GRID_SIZE and
                ...
                if (self.board.get_piece((bx1, by1)) == opponent and
                    ...
                    if (bx1, by1) not in [p for p, _ in result]:
                        result.append(((bx1, by1), f"规则b: 方向({dx},{dy}) 敌方-己方-敌方 (左)"))
                    if (bx2, by2) not in [p for p, _ in result]:
                        result.append(((bx2, by2), f"规则b: 方向({dx},{dy}) 敌方-己方-敌方 (右)"))
    ...
    def _get_flips_for_trigger(self, player: int, pos: Tuple[int, int]) -> List[Tuple[Tuple[int, int], str]]:
        ...
        for dx, dy in directions:
            ...
            if (0 <= ax1 < GRID_SIZE and 0 <= ay1 < GRID_SIZE and
                ...
                if (self.board.get_piece((ax1, ay1)) == opponent and
                        self.board.get_piece((ax2, ay2)) == player):
                    result.append(((ax1, ay1), f"连锁-规则a 触发点({px},{py})"))
            ...
            if (0 <= bx1 < GRID_SIZE and 0 <= by1 < GRID_SIZE and
                ...
                if (self.board.get_piece((bx1, by1)) == opponent and
                        self.board.get_piece((bx2, by2)) == opponent):
                    if (bx1, by1) not in [p for p, _ in result]:
                        result.append(((bx1, by1), f"连锁-规则b 触发点({px},{py})"))
                    if (bx2, by2) not in [p for p, _ in result]:
                        result.append(((bx2, by2), f"连锁-规则b 触发点({px},{py})"))
...
class ChainFlipHandler:
    ...
    def select_trigger(self, pos: Tuple[int, int]):
        ...
        if pos in self.available_triggers:
            ...
            self.preview_flips = [p for p, _ in self.flip_rule.get_flips_for_trigger(self.player, pos)]

    def apply_trigger(self) -> bool:
        """应用选中的触发点翻转，返回是否还能继续连锁"""
        if self.selected_trigger is None:
            return False

        # 获取要翻转的棋子
        flips = self.flip_rule.get_flips_for_trigger(self.player, self.selected_trigger)

        # 执行翻转（规则b两边同时翻转）
        for pos, reason in flips:
...
class Game:
    ...
    def move_to(self, pos: Tuple[int, int]) -> bool:
        ...
        if self.pending_flips:
            self.state = Game.FLIPPING
            self.selected_pos = pos
            self.logger.log(f"可翻转: {[p for p, _ in self.pending_flips]}")
        else:
            self._end_turn()
    ...
    def apply_flips(self):
        ...
        for pos, reason in self.pending_flips:
    ...
    def chain_apply(self):
        ...
        has_more = self.chain_handler.apply_trigger()
        self.logger.log_board_state(self.board.grid)
...
class GUI:
    ...
    def draw_highlight(self):
        ...
        if self.game.state == Game.FLIPPING:
            for pos, _ in self.game.pending_flips:
                center = self.pos_to_screen(pos)
                pygame.draw.circle(self.screen, HIGHLIGHT_COLOR, center, 30, 4)
        ...
        if self.game.state == Game.CHAIN_FLIPPING:
            ...
            selected = self.game.chain_handler.selected_trigger
    ...
    def draw_ui(self):
        ...
        if self.game.state == Game.GAME_OVER:
            ...
            if self.game.state == Game.SELECTING:
                ...
                msg = f"{player_name} - 点击确认翻转"
            elif self.game.state == Game.CHAIN_FLIPPING:
                msg = f"{player_name} - 点击己方棋子(黄框)预览, 确认/跳过"
            else:
                msg = ""
